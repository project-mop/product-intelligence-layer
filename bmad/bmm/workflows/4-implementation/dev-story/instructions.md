...
Expected ready-for-dev or in-progress. Continuing anyway...
  ...
  <step n="3" goal="Author comprehensive tests">
    <action>BEFORE writing tests, consult testing strategy: /docs/testing-strategy-mvp.md</action>
    <action>Identify story complexity:
      - Simple CRUD (1-3 fields, basic validation) → 50% coverage, standard timeouts
      - Complex Forms (dynamic arrays, async validation, React Hook Form) → See "Advanced Testing Patterns" section
      - API endpoints → Test success/error cases, authorization, input validation
    </action>
  ...
  <step n="5" goal="Mark task complete, track review resolutions, and update story">
    ...
    <action if="no tasks remain"><goto step="5.5">Test validation gate</goto></action>
  </step>

  <step n="5.5" goal="Validate all tests pass locally before code review" tag="quality-gate">
    <critical>QUALITY GATE: Story cannot proceed to review with failing tests</critical>
    <critical>This step prevents Story 1.6-style issues where untested code reaches review</critical>

    <action>Navigate to project root</action>
    <action>Run: npm run test:unit</action>

    <check if="tests fail">
      <output>❌ **Tests Failed - Cannot Proceed to Review**

**Failing Tests Detected:**
Story cannot be marked "ready for review" with failing tests.

**Next Steps:**
1. Run tests locally to see failures:
   npm run test:unit -- --verbose

2. Fix failing tests following troubleshooting guide:
   See /docs/testing-strategy-mvp.md "Troubleshooting Test Failures"

3. Common Issues (React Hook Form):
   - Async validation → Increase waitFor timeout to 5000ms
   - Dynamic array fields → Add explicit user.tab() calls
   - Expensive Zod .refine() → Move to component-level validation

4. Re-run this workflow after tests pass

**Reference Documents:**
- Testing Strategy: /docs/testing-strategy-mvp.md
- Story 1.6 Retrospective: /docs/stories/retrospective-1.6-testing-crisis.md
      </output>
      <action>HALT</action>
    </check>

    <check if="tests pass">
      <output>✅ All tests passing - Ready for code review

**Test Validation Complete:**
- All unit tests passing
- Ready to proceed to completion checks
- Next step: Verify story completion criteria
      </output>
